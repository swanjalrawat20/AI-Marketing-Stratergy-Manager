...
.metric-card {
    ...
    border-radius: 16px;
    padding: 21px;
    box-shadow: 0 5px 18px rgba(80,55,40,.04);
}
...
.strategy-engine {
    ...
    border-radius: 16px;
    padding: 21px;
    box-shadow: 0 9px 25px rgba(201,106,61,.18);
}
...
def store_campaign(campaign: dict) -> None:
    ...
    if existing:
        ...
        for key in ("strategy_history", "feedback_history", "approval_history"):
            campaign[key] = list(existing.get(key) or campaign.get(key) or [])
...
Human reviewer revision feedback:
{feedback.strip()}

Update the strategy specifically in response to this feedback.
""".strip()
...
def dashboard():
    ...
    with left:
        ...
        y = [
    round(min(7 * i, days) * c["daily_signup_target"], 1)
    for i in range(1, points + 1)
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=y,
                mode="lines+markers",
                line=dict(color="#23120B", width=3),
                marker=dict(color="#1D1B1A", size=7),
                fill="tozeroy",
        ...
        fig.update_layout(
            ...
            paper_bgcolor="#140E03",
            plot_bgcolor="#0E0A02",
            font=dict(family="Arial", color="#27180E"),
            xaxis=dict(showgrid=False, zeroline=False),
            yaxis=dict(showgrid=True, gridcolor="#1E140B", zeroline=False),
            showlegend=False,
        )
    ...
    with right:
        ...
        if st.button("＋  Open Campaigns", use_container_width=True):
            navigate("Campaigns")
            st.rerun()
...
def campaigns():
    ...
    if st.button(
        "Continue to AI Strategy Studio →",
        use_container_width=True,
        key="continue_to_strategy_after_create",
    ):
        navigate("Strategy Studio")
        st.rerun()
...
def approval_center():
    ...
    with right:
        ...
        human_color = "#638058" if approved else "#735833"
        ...
        html(
            ...
            <div class="section-card">
                ...
                <div class="check-item">
                    ...
                    <div class="check-status" style="color:#638058;">
                        Ready
                    </div>
                </div>
